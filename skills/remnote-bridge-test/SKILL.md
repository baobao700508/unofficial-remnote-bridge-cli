...
subagent 返回后，**立即**将其完整输出保存到 `{workspace}/{test_id}-{interface}/transcript.md`。不要等到最后统一保存。
...
7. **立即**将截图保存到 `{workspace}/{test_id}-{interface}/screenshot-{N}.png`
...
**立即**将判定结果追加到 `{workspace}/results.md`（格式见 §6）。每完成一个用例就追加，不要等所有用例跑完再统一写。
...
Agent(
  ...
  - Tool 修复如果改了 Plugin 必须 bump 版本号（manifest.json + package.json + settings.ts）
  - 修完后用 npm run build 确认编译通过
  """
)
```
...
修复后**必须**回到 Step 2，重新派 haiku subagent 跑同一个用例。
...
**1. 编译 + 重启（每次测试前必须执行）**
```
a. npm run build                       ← 编译最新代码
b. disconnect()                        ← 停掉旧的 daemon（如果在运行的话）
c. connect()                           ← 重新启动（⛔ 不带参数，禁止 headless）
d. 用 Claude in Chrome 导航到 RemNote 并刷新页面  ← 你自己刷新，禁止叫用户操作
e. health 确认三层就绪
f. 不通 → 再刷新一次 → 再 health → 仍不通则停止，报告连接问题
```
