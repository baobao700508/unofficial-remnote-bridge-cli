...
| 当前页面内容 | `read-context`（默认 page 模式） | 均匀展开，**无缓存**。用户通常只是浏览页面，此模式最可靠 |
| 我选中的 Rem 附近 | `read-context --mode focus` | 鱼眼视图（焦点 depth=3，siblings depth=1，叔伯 depth=0），**无缓存**。仅在用户明确说"我选中的/光标在的" |
