...
│   ├─ page 模式（默认）：以当前打开页面为根展开子树。用户通常只是浏览页面，此模式最可靠
│   └─ focus 模式：以用户聚焦的 Rem 为中心的鱼眼视图（仅在用户明确说"我选中的 Rem"时使用）
...
| "当前页面的内容" | `read-context`（默认 page 模式） | 以页面为根展开 |
| "我选中的 Rem 附近" | `read-context --mode focus` | 鱼眼视图，焦点处详细 |
...
- 用户通常只是打开页面浏览，不会特意聚焦到某个 Rem，此模式最可靠

**focus 模式**（仅在用户明确提到"我选中的 Rem"、"我光标在的位置"时使用）：
