...
- **page 模式**（默认）：以当前打开的页面 Rem 为根，均匀展开子树。用户通常只是打开页面浏览，不会特意聚焦到某个 Rem，此模式最可靠
- **focus 模式**：以当前焦点 Rem 为中心，向上追溯祖先，构建鱼眼视图——焦点完全展开，周围递减。仅在用户明确提到"我选中的 Rem"、"我光标在的位置"时使用
...
| 字段 | 类型 | 必需 | 说明 |
|------|------|:----:|------|
| `mode` | string | 否 | 模式：`"page"`（默认）或 `"focus"` |
