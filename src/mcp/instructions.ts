...
| 修改引用列表（增删引用的 Rem） | \\\`edit_rem\\\` | 修改 changes 中的 \\\`portalDirectlyIncludedRem\\\` 数组 |
...
- **端口残留**：多次 connect 失败后可能出现端口被占用（EADDRINUSE），用 \\\`disconnect\\\` 或手动终止占用端口的进程后重试
...
1. \\\`read_rem\\\` 获取目标 Rem 的属性（建立缓存）
2. 查看返回的属性，确定要修改的字段和新值
3. \\\`edit_rem\\\` 直接传入 changes 对象：\\\`{字段名: 新值}\\\`

示例——修改类型为 concept：
\\\`\\\`\\\`
edit_rem(remId, changes: {type: "concept"})
\\\`\\\`\\\`

示例——设置黄色高亮：
\\\`\\\`\\\`
edit_rem(remId, changes: {highlightColor: "Yellow"})
\\\`\\\`\\\`

示例——修改文本为粗体：
\\\`\\\`\\\`
edit_rem(remId, changes: {text: [{"b": true, "i": "m", "text": "粗体标题"}]})
\\\`\\\`\\\`

示例——给文本加超链接：
\\\`\\\`\\\`
edit_rem(remId, changes: {text: ["点击", {"i": "m", "iUrl": "https://remnote.com", "text": "访问官网"}]})
\\\`\\\`\\\`
...
| 属性 | 位置 | 值类型 | 效果 | 修改方式 |
|:-----|:-----|:-------|:-----|:---------|
| \\\`highlightColor\\\` | RemObject 顶层字段 | 字符串 \\\`"Red"\\\`/\\\`"Yellow"\\\` 等，或 \\\`null\\\` | 整行背景色（左侧彩色竖条） | changes 直接设置 |
| \\\`h\\\` | RichText 元素内部 | 数字 0-9 | 文字片段的荧光底色 | changes.text 整体替换 |
...
### 常见属性修改速查（edit_rem changes 示例）

**① 设置/清除整行背景色（highlightColor）**
\\\`\\\`\\\`
changes: {highlightColor: "Yellow"}     // 设置
changes: {highlightColor: null}         // 清除
\\\`\\\`\\\`

**② 修改 Rem 类型**
\\\`\\\`\\\`
changes: {type: "concept"}              // 设为概念
changes: {type: "default"}              // 恢复普通
\\\`\\\`\\\`

**③ 修改文字颜色（RichText tc 字段）**

需要在 text 数组中的对象元素添加 tc 字段，通过 changes.text 整体替换 text 数组。
...
| 工具 | 缓存行为 | 用途 |
|:-----|:---------|:-----|
| \\\`read_rem\\\` | 写入缓存 | 供 \\\`edit_rem\\\` 的防线检查使用 |
...
此外，edit_rem 使用字段白名单校验：只读字段和未知字段会产生警告，不会阻断操作。
...
| Invalid value for 'field' | 枚举字段值不合法 | 检查字段的合法值（如 type 只接受 concept/descriptor/default） |
| Field '...' is unknown and was ignored | 未知字段警告 | 检查字段名拼写，参考 read_rem 返回的字段 |
| old_str not found（edit_tree） | oldStr 在大纲中不存在 | 检查是否精确匹配（含缩进、空格、换行） |
| old_str matches N locations（edit_tree） | oldStr 匹配多处 | 扩大 oldStr 范围，包含更多上下文以唯一定位 |
