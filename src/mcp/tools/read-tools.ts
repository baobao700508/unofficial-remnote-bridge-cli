...
export function registerReadTools(server: FastMCP): void {
  ...
  server.addTool({
    ...
    description:
      ...
      '\\n\\n模式选择：一般情况下用户只是打开了页面在浏览，并不会特意聚焦到某个 Rem，所以默认使用 page 模式。仅当用户明确说"我选中的 Rem"、"我光标在的位置"、"当前聚焦的"时才使用 focus 模式。' +
      '\\n\\n适用场景：了解用户当前打开的页面或焦点位置；用户说"我现在在看什么"、"当前页面是什么"时使用；需要上下文才能理解用户指代时使用。' +
      '\\n不适用场景：查看特定 Rem（已知 remId 用 read_tree）；搜索内容（用 search）。' +
      '\\n\\n前置条件：daemon 已连接。page 模式需有打开的页面；focus 模式需用户在 RemNote 中有焦点 Rem（光标在某个 Rem 上）或指定 focusRemId。' +
      '\\n\\n参数说明：' +
      '\\n- mode（可选，默认 "page"）：视图模式' +
      '\\n  - page（默认）：以当前打开的页面为根，均匀展开子树。用户通常只是打开页面浏览，此模式最可靠' +
      '\\n  - focus：以焦点 Rem 为中心的鱼眼视图。焦点完全展开(depth=3)，siblings 浅层预览(depth=1，前3个children可见)，叔伯不展开。焦点行以 * 前缀标记。需要用户光标在某个 Rem 上' +
    ...
    parameters: z.object({
      mode: z
        .enum(['focus', 'page'])
        .optional()
        .describe('视图模式：page（页面，默认）或 focus（聚焦，需用户光标在某个 Rem 上）'),
      ancestorLevels: z
        .number()
        .optional()
        .describe('向上展示的祖先层级数'),
