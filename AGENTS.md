...
- **项目是什么**：RemNote 自动化桥接工具集（remnote-bridge），将 RemNote 知识库能力暴露给 AI Agent。单包发布，`npm install -g remnote-bridge` 后即可使用。
- **核心架构**：三层协作——remnote-plugin（桥接层）→ src/cli（命令层）→ src/mcp + skill/（接入层）。单包 ESM 模块。
...
AI Assistants (Claude Code / OpenClaw / etc.)
    ↕
skill/ (Markdown Skills)  +  src/mcp/ (FastMCP Server)
    ↕                             ↕
                  src/cli/ (核心命令层)
                      ↕
...
| 层 | 目录 | 语言/框架 | 状态 |
|:--|:--|:--|:--|
| 桥接层 | remnote-plugin/ | Node.js / TypeScript / RemNote Plugin SDK | 开发中 |
...
| 层 | 目录 | 职责 | 禁止事项 |
|:--|:--|:--|:--|
| 桥接层 | remnote-plugin/ | 通过 RemNote SDK 与知识库交互，暴露 WebSocket API | 禁止包含 CLI 逻辑或 MCP 协议代码 |
| 命令层 | src/cli/ | 封装 RemNote 操作为统一命令接口 | 禁止直接调用 RemNote SDK（必须通过 remnote-plugin） |
| 接入层 | skill/ | 定义 Agent 可调用的技能（Markdown） | 禁止包含运行时代码逻辑 |
| 接入层 | src/mcp/ | 通过 MCP 协议暴露 RemNote 操作为标准工具 | 禁止绕过 src/cli 直连 remnote-plugin |
...
- **禁止**：remnote-plugin 依赖 src/cli 或 src/mcp
- **禁止**：src/cli 依赖 src/mcp
- **允许**：src/mcp 通过子进程调用 src/cli（不通过 import）
...
- **检查工具**：`node scripts/check-layer-deps.cjs`

#### 2.1.4 CLI 内部分层（强约束）

src/cli 内部按语义分为**基础设施**和**业务编排**两类：
...
- **检查工具**：`node scripts/check-layer-deps.cjs`（同时检查跨层、Plugin 内部和 CLI 内部依赖）
...
├── package.json                       (单包，bin: remnote-bridge)
├── tsconfig.json                      (CLI + MCP 统一编译)
├── src/
│   ├── cli/                           ← 命令层（原 remnote-cli/src/）
│   │   ├── main.ts                    # 伞入口（+mcp/install 子命令）
...
│   │   ├── protocol.ts               # WS 协议类型
│   │   └── utils/                     # 输出格式化
│   └── mcp/                           ← 接入层（原 remnote-mcp/src/）
│       ├── index.ts                   # 导出 startMcpServer()
│       ├── daemon-client.ts           # CLI 子进程调用封装
...
本项目中 remnote-plugin、src/cli、src/mcp、skill 统一称为**"层"**，不称"模块"。因为它们是横向分割的架构分层，不是纵向分割的功能模块。

| 称呼 | 对应目录 | 角色 |
|:--|:--|:--|
| 桥接层 | `remnote-plugin/` | 通过 RemNote SDK 与知识库交互 |
| 命令层 | `src/cli/` | 封装操作为统一 CLI 命令 |
| 接入层 | `src/mcp/` + `skill/` | 暴露给 AI Agent（MCP / Skills） |
