...
```
unofficial-remnote-bridge read-rem <remId> [--fields text,tags,type] [--full] [--json]
```
...
```
unofficial-remnote-bridge edit-rem <remId> --old-str <old_str> --new-str <new_str> [--json]
```
...
```
unofficial-remnote-bridge edit-rem rem_abc123 \
  --old-str '"机器学习是一种"' \
  --new-str '"深度学习是一种"'
```
...
```
unofficial-remnote-bridge edit-rem rem_xyz ...
→ 检查缓存：rem_xyz 不存在
...
```
unofficial-remnote-bridge edit-rem rem_abc123 --old-str ... --new-str ...
→ 第一道防线通过（缓存存在）
