<!-- source: https://plugins.remnote.com/api/classes/CardNamespace -->
<!-- crawled: 20260302_170143 -->

-   [](/)
-   API Reference
-   Namespaces
-   CardNamespace

On this page

# Class: CardNamespace

## Hierarchy[​](#hierarchy "Direct link to heading")

-   `NamespaceBase`
    
    ↳ **`CardNamespace`**
    

## Methods[​](#methods "Direct link to heading")

### findMany[​](#findmany "Direct link to heading")

▸ **findMany**(`ids`): `Promise`<[`Card`](/api/classes/Card)\[\]\>

#### Parameters[​](#parameters "Direct link to heading")

Name

Type

`ids`

`string`\[\]

#### Returns[​](#returns "Direct link to heading")

`Promise`<[`Card`](/api/classes/Card)\[\]\>

* * *

### findOne[​](#findone "Direct link to heading")

▸ **findOne**(`id?`): `Promise`<`undefined` | [`Card`](/api/classes/Card)\>

#### Parameters[​](#parameters-1 "Direct link to heading")

Name

Type

`id?`

`string`

#### Returns[​](#returns-1 "Direct link to heading")

`Promise`<`undefined` | [`Card`](/api/classes/Card)\>

* * *

### getAll[​](#getall "Direct link to heading")

▸ **getAll**(): `Promise`<[`Card`](/api/classes/Card)\[\]\>

#### Returns[​](#returns-2 "Direct link to heading")

`Promise`<[`Card`](/api/classes/Card)\[\]\>